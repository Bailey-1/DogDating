...
async function showMessages() {
  ...
  recieverProfile = await util.getProfileById(window.location.hash.substring(1));
  currentProfileData = await util.getProfileById(util.currentProfile);

  recieverProfile = recieverProfile[0];
  currentProfileData = currentProfileData[0];

  // Get message object for all messages between them
  messageObj = await util.getMessages(recieverProfile.pro_id, currentProfileData.pro_id);
...
async function sendMessageProperties() {
  ...
  console.log('id: ', util.currentProfile);
  console.log('recId: ', recId);
  console.log('msg: ', msg);
  if (msg) {
    const result = await util.sendMessage(util.currentProfile, recId, msg);
    console.log(result);
    const newMessage = await util.getMessage(result);
...
async function mainLoop() {
  ...
  // Get message object for all messages between them
  const newMessages = await util.getMessages(recieverProfile.pro_id, currentProfileData.pro_id);
