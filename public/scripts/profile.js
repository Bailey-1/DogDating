...
async function showProfileImages() {
  ...
  const id = getProfile();
  const imageObj = await util.getImagesById(id);
...
async function generateReviewElement(reviewObj) {
  ...
  const starLocation = clone.querySelector('#reviewRating');
...
async function sendReview() {
  const body = { content: '', rating: 0 };
  body.content = document.querySelector('#reviewTextArea').value;
  body.rating = currentRating;
  await util.postReview(util.currentProfile, window.location.hash.substring(1), body);
  getReviews();
}
...
function clearStars() {
  ...
  for (const element of starElements) element.src = './svg/line-star.svg';
}
...
function rateStars(numOfStars) {
  ...
  for (const element of starElements) element.src = './svg/fill-star.svg';
}
