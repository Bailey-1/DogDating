...
async function createProfileElement(profile) {
  ...
  clone.querySelector('#name').textContent = `${profile.pro_name}, ${util.getAgeFromDate(profile.pro_birthday)}`;
...
async function loadFilters() {
  const filters = await util.getFilters(util.currentProfile);
  ...
  for (const option of filters.kennelClubMembership.rows) { createOptions('select-kennelclub', option.membership_type); }
...
async function updateProfilesSelection() {
  const queryArr = [];
  // Filters
  if (document.querySelector('#select-location').value !== 'all') { queryArr.push('location=' + document.querySelector('#select-location').value); }
  if (document.querySelector('#select-breed').value !== 'all') { queryArr.push('breed=' + document.querySelector('#select-breed').value); }
  if (document.querySelector('#select-kennelclub').value !== 'all') { queryArr.push('kc=' + document.querySelector('#select-kennelclub').value); }
  // Sort
